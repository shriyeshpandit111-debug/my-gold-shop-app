...
def prepare_orderflow_frame(df, min_rows=20, allow_real_trade_flow=False):
    """Prepare one synchronized candle frame for Tab 6.

    IMPORTANT: BTC may use real Binance aggressor-side executed trades.
    GOLD/SILVER/NIFTY/BANKNIFTY and other OHLCV-only feeds MUST recompute
    delta from the exact same candle shown in the price chart. This prevents
    an old/precomputed delta column from becoming detached from the candle.

    OHLCV candle-delta proxy formula:
        signed_ratio = (Close - Open) / (High - Low)
        delta = volume * signed_ratio
        buy = (volume + delta) / 2
        sell = (volume - delta) / 2

    This is a candle-flow proxy, not exchange-level aggressor volume.
    """
    if df is None or df.empty:
        return None
    ...
    # ONLY a genuine trade-flow source is allowed to reuse buy/sell/delta.
    # In particular, never reuse precomputed proxy columns for GOLD/SILVER.
    has_trade_columns = all(c in out.columns for c in ["buy_vol", "sell_vol", "delta"])
    real_trade_flow = bool(allow_real_trade_flow and has_trade_columns)
    ...
    if real_trade_flow:
        ...
        # Recalculate from the two real sides so delta can never be stale.
        out["delta"] = out["buy_vol"] - out["sell_vol"]
        out["display_volume"] = out["volume"]
        out["volume_label"] = "Exchange Volume"
        out["delta_method"] = "Binance executed trades (aggressor side)"
    else:
        # GOLD/SILVER and other non-BTC markets: delta MUST come from the
        # same OHLCV candle displayed directly above the delta bars.
        out["flow_source"] = "OHLCV_CANDLE_DELTA_PROXY"
        rng = (out["high"] - out["low"]).clip(lower=0.0)
        body = out["close"] - out["open"]

        # For normal candles, scale volume by the candle body/range.
        # For zero-range candles, use candle direction; doji => zero delta.
        safe_rng = rng.replace(0.0, np.nan)
        signed_ratio = (body / safe_rng).replace([np.inf, -np.inf], np.nan).fillna(0.0)
        zero_range = rng <= 0.0
        signed_ratio = signed_ratio.where(
            ~zero_range,
            np.sign(body).astype(float)
        ).clip(-1.0, 1.0)

        zero_volume = float(out["volume"].abs().sum()) <= 0.0
        if zero_volume:
            # Some feeds can return zero volume. Keep a deterministic activity
            # proxy for the bottom panel, but delta still follows candle body.
            ...
            delta_base = out["display_volume"]
        else:
            out["display_volume"] = out["volume"]
            out["volume_label"] = "Candle Volume"
            delta_base = out["volume"]

        # This is the exact candle delta used by the Tab-6 delta bars.
        out["delta"] = (delta_base * signed_ratio).astype(float)
        out["buy_vol"] = ((delta_base + out["delta"]) / 2.0).clip(lower=0.0)
        out["sell_vol"] = ((delta_base - out["delta"]) / 2.0).clip(lower=0.0)
        out["delta_method"] = "Candle Delta = Volume × (Close−Open)/(High−Low)"
...
with tab6:
    ...
    if is_gold_silver:
        try:
            ...
            pass
    flow_df = prepare_orderflow_frame(
        flow_df, min_rows=72, allow_real_trade_flow=is_btc_market
    )
    ...
    if flow_df is not None and len(flow_df) >= 2:
        ...
        st.caption(
            f"Flow source: {flow_source_name} | Volume panel: {last['volume_label']} | "
            f"Delta method: {last['delta_method']} | Timestamp display: Asia/Kolkata (IST)"
        )

...
def get_upcoming_expiry_date():
    ...
    # आजचा वार शोधणे (Monday=0, ... Thursday=3, ... Sunday=6)
    # जर आज गुरुवार असेल तर आजची तारीख, नाहीतर पुढील गुरुवार शोधणे
...
def get_upstox_option_chain_data(inst_key, token):
    ...
    try:
        ...
        # २. जर डेटा नाही मिळाला किंवा रिकामी लिस्ट आली, तर रिकामी एक्सपायरी (Fallback) पाठवून API वर सोडूया
        is_empty = False
        if response.status_code == 200:
            res_data = response.json()
...
if access_token:
    ...
    st.warning("👈 डाव्या बाजूला आधी तुमचा 'Upstox Access Token' टाका, म्हणजे डेटा लोड होईल.")import streamlit as st
import pandas as pd
import numpy as np
import plotly.graph_objects as go
import requests
